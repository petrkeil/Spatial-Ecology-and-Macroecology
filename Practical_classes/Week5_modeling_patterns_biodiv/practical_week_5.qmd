...
The point of this class is to put you in the situation where you have to download, explore and analyse a large biodiversity dataset. It is made to show you the steps that you are likely to follow when you are working with macroecological datasets. We will work with one of the most famous and high quality ecological dataset: the North American Breeding Bird Survey (BBS).
...
[The North American BBS](https://www.pwrc.usgs.gov/bbs/) is an open source dataset that you can dowload directly [here](https://www.sciencebase.gov/catalog/item/52b1dfa8e4b0d9b325230cd9). In `Child Items --> Historic data release`, click `2022 Release - North American Breeding Bird Survey Dataset (1966-2021)` and under `Attached Files`, click on `download all`. The download should start few seconds after. Save the `.zip` archive into a `data` folder that you created in your working directory.
...
Let's visualize the spatial coverage of the dataset. Information about the roads can be found in the `routes.csv` file. First, explore the metadata file that you previously opened in your browser and find the information about the `road.csv` file.
...
Species richness represents the total number of species at one location. It is one of the most commonly used metric of biodiversity as it is really straight forward to assess. Data about the species can be found in folder `50-StopData/1997ToPresent_SurveyWide/`. They are spread on 10 excel spreadsheets. In order to not repeat 10 times the loading process, we will automate it:
...
With $n$ the number of covariates. Thus, this equation can be represented in an (n+1)-dimensional space and represent a subspace of $n$ dimensions called an **hyperplane**. Moreover, the values of the $\beta$s will indicate both the sign and the strenght of the relationship (*e.g.* $\beta_1$ will show if there is a negative/positive influence of $x_1$ on $y$). The goal of fitting a linear regression (but also of most of the machine learning algorithms) is to find the hyperplane which fits the most to $y$ (*i.e.* which explains most of its variance). In machine learning, you do so by minimizing a *loss function*. The loss function for a linear regression is called the **Residual Sum of Square** (*i.e. RSS*).

$$RSS = \sum _{i = 1}^j (y_i - \hat y_i)^2$$ with $j$ the number of observations in your dataset, $y$ your observed data and $\hat{y}$ is the predicted value. Here, we will use weather data as covariates (*i.e.* $x_1\ldots x_n$) in order to predict the species richness for year 2019. Weather data is stored in the `weather.csv` file.
...
```{r}
## First we can filter the weather data to only keep the info for 2019
...
plot(lm_data$sr_2019, lm_model$fitted.values)
abline(0,1)
```
...
```{r}
library(Metrics)
rmse(lm_data$sr_2019, lm_model$fitted.values)
```

# Exercice 1: fitting a GLM (Poisson regression)

Species richness is a count of species. Counts can't be negative so fitting a linear regression for counts is not a good practice (because it can predict negative values). **Fit a poisson regression** using the `glm()` function. Look at the `summary()` of the model and compute the $RMSE$. **What can you say about the model? Is it better/worse/same than the linear model?** 

# Exercice 2: assessing drivers of Shannon index and evenness
