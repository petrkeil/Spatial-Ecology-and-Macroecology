...
The point of this class is to put you in the situation where you have to download, explore and analyse a large biodiversity dataset.
...
The main assumption is that there is a linear relationship between the variable you want to explain (*a.k.a.* the target, the predicted variable, the output, dependent variable)
and a set of explaining variables (*a.k.a.* predictors, covariates, features, independent variables).
For instance today, we will try to explain how species richness is driven by several weather variables. 
That is: species richness is the target while the weather parameters are the predictors.
...
Thus, this equation can be represented in an (n+1)-dimensional space (called an hyperspace when $n+1 > 3$).
Moreover, the values of the $\beta$s will indicate both the sign and the strenght of the relationship (*e.g.* $\beta_1$ will show if there is a negative/positive influence of $x_1$ on $y$).
The goal of fitting a linear regression (but also of most of the machine learning algorithms) is to find the hyperplane which fits the most to $y$ (*i.e.* which explains most of its variance).
...
$$RSS = \sum _{i = 1}^j (y_i - \hat y_i)^2$$
with $j$ the number of observations in your dataset.
Here, we will use weather data as covariates (*i.e.* $x_1\ldots x_n$) in order to predict the species richness for year 2019 ($y$).
