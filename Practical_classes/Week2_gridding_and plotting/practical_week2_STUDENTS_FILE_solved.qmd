...
```{r}
trees <- st_read('Practical_classes/Week2_gridding_and plotting/data/trees.gpkg', quiet=T)
```
...
```{r}
mammalsCZ <- readRDS('Practical_classes/Week2_gridding_and plotting/data/mammalsCZ.rds')
```
...
```{r}
CZ_borders <- st_read('Practical_classes/Week2_gridding_and plotting/data/CZE_adm0.gpkg')
```
...
```{r}
CZ_borders <- st_transform(CZ_borders, crs = 'EPSG:5514') 
```

Create a grid layer of 100 km^2^ area (10x10), with the extent of `CZ_borders`

```{r}
CZ_grids <- st_make_grid(CZ_borders, 
                         cellsize=10000) %>%
  st_intersection(CZ_borders) %>% 
  st_sf(gridID=1:length(.)) %>% st_cast()     
```
...
```{r}
# mammalsCZ_sf <- st_transform(mammalsCZ_sf, crs = 'EPSG:5514') 
...
ggplot() + 
  ...
  scale_fill_fermenter(palette ='YlOrBr', n.breaks=8, direction = 1) + # fill of the grids
