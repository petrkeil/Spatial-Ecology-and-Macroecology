...
To easily align the figures in a composition we will also use `patchwork`.

```{r}
#| echo: true
#| eval: false
install.packages('patchwork') # install 
```
...
[We'll see how to get this polygon data directly from R later]{.font-aside}
...
[We'll see how to create this grid layer directly from R later]{.font-aside}
...
[We will see how to account for this issue later, while we wait for some geoprocessing steps :) ]{.font-aside}
...
We will download a shapefile of the world at medium scale and we will combine all countries into a unique polygon
...
<br> Earth is not flat :) **Projections** help us represent the two-dimensional curved surface of the globe into 2D space. There are many ways to do this.
...
<br> We have the grid, we are ready to calculate richness. Let's see how it looks
