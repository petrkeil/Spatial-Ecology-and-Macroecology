...
# Simulating macroevolutionary and biogeographic dynamics across space and time

Gen3sis (Hagen et al.) provides a framework to model how species evolve,
interact with their environment, disperse, and diversify across
landscapes. gen3sis is especially useful for studying species
diversification, speciation, extinction, and migration under varying
environmental and geological conditions.

::: callout-note
## ⭐Key Features

-   ***Species Evolution and Diversification***: Simulates speciation
    events based on user-defined rules.
...
2.  The time-series of the **landscape** in which these processes happen
    (including plate tectonics, mountain building, sea level changes,
    etc.)\
    *Example resources*:
...
open up a config.R file from your file explorer. The second way includes
loading the file into R as config for gen3sis and adjust it in R. To
keep our simulations reproducible and all changes we make to the config,
we will modify the file from within R.
:::

```{r}
# gen3sis-function to initialize the config file (to tell the model what to use)
config_raw <- create_input_config(config_file = path_config)

```

We can edit config files like this:

```{r}
#| eval: false
file.edit(here(data_path, "config" , "config_sim1.R"))
```
...
The landscape is made up of 2 classes of files:

1\) The landscape object with latitude, longitude and environmental
variables;

2\) The distance matrices that set the dispersal options for species in
the landscape.
...
should be infinite (water-species do not disperse over land).

The distances are calculated during the creation of the world with the
function:
`create_input_landscape(…,calculate_full_distance_matrices = TRUE)`
:::
...
# Plotting the areas
plot(terra::rast(landscape$area[, c("x", "y", "66")]), 
     main = "Area 66 Million years ago")

plot(terra::rast(landscape$area[, c("x", "y", "30")]), 
     main = "Area 30 Million years ago")

plot(terra::rast(landscape$area[, c("x", "y", "1")]), 
     main = "Area 1 Million years ago")



# Plotting temperature with custom colors
plot(terra::rast(landscape$temp[, c("x", "y", "66")]), 
     main = "Temperature 66 Million years ago", 
     col = temp_colors(100))

plot(terra::rast(landscape$temp[, c("x", "y", "30")]), 
     main = "Temperature 30 Million years ago", 
     col = temp_colors(100))

plot(terra::rast(landscape$temp[, c("x", "y", "1")]), 
     main = "Temperature 1 Million years ago", 
     col = temp_colors(100))



# Plotting aridity with custom colors
plot(terra::rast(landscape$arid[, c("x", "y", "66")]), 
     main = "Aridity 66 Million years ago", 
     col = arid_colors(100))

plot(terra::rast(landscape$arid[, c("x", "y", "30")]), 
     main = "Aridity 30 Million years ago", 
     col = arid_colors(100))

plot(terra::rast(landscape$arid[, c("x", "y", "1")]), 
     main = "Aridity 1 Million years ago", 
     col = arid_colors(100))
...
plot(seq(from = 1, to = 66, by = 1), 
     mean_temperatures, 
     xlab = "time (Ma)", 
     ylab = "mean temperature", 
     type = "l")

# plot the change in mean aridity through time
...
This simulation runs 10 minutes on my machine. You should at least watch
a simulation once and see what happens through time :)
...
However, if time runs low, we can also read in the results from the .rds
object automatically saved to the output folder as '*sgen3sis.rds*'.

```{r}
sim0 <- readRDS(here(data_path, "output", "sim0", "config_southamerica", "sgen3sis.rds" ))
plot_summary(sim0)
```

### 🖼️Plot SR for 3 time steps
...
for (i in timesteps) {
  landscape_i <- readRDS(
    file.path(
      data_path,
      paste0(
        "output/sim0/config_southamerica/landscapes/landscape_t_",
        i, ".rds"
      )
    )
  )
  species_i <- readRDS(
    file.path(
      data_path,
      paste0(
        "output/sim0/config_southamerica/species/species_t_",
        i, ".rds"
      )
    )
  )
...
# ❇️Activity: Modify the simulation

Let's change some of the parameters and **compare the species richness**
between different configurations.

::: callout-caution
## ⚠️Caution⚠️

Some parameters will make the model run for a very long time. If your
model has not finished after 5-10 minutes (max!), it's time to stop.
:::
...
Now you would usually open the config.R file and replace the function in
the code with your adjustments. If you want to play around with the
numbers, you can do it just like this:
...
tictoc::tic("sim1")
sim1 <- run_simulation(
  config = file.path(data_path, "config/config_sim1.R"),
  landscape = file.path(data_path, "landscape"),
...
::: callout-important
## New create_ancestor_species function:
...
# Run the simulation with the new configuration
sim2 <- run_simulation(
  config = file.path(data_path, "config/config_sim2.R"),
  landscape = file.path(data_path, "landscape"),
...
Change the arguments of the Weibull distribution to achieve a [lower
dispersal distance]{.underline} for the model. What happens?

*Caution: Increasing the dispersal distance increases computing time a
lot!*

*scale = 200 took me 25 minutes to simulate.*

*For time reasons, we should stick to decreasing it during the practical
course.*
:::
...
::: callout-important
## New get_dispersal_values function:
...
# Run the simulation with the new configuration =========
sim3 <- run_simulation(
  config = file.path(data_path, "config/config_sim3.R"),
  landscape = file.path(data_path, "landscape"),
...
Change the divergence time to a [longer]{.underline} time interval. What
happens?
...
*For time reasons, we should stick to increasing it during the practical
course.*
:::
...
# Run the simulation with the new configuration
sim4 <- run_simulation(
  config = file.path(data_path, "config/config_sim4.R"),
  landscape = file.path(data_path, "landscape"),
...
# Run the simulation with the new configuration
sim5 <- run_simulation(
  config = file.path(data_path, "config/config_sim5.R"),
  landscape = file.path(data_path, "landscape"),
...
# Run the simulation with the new configuration
sim6 <- run_simulation(
  config = file.path(data_path, "config/config_sim6.R"),
  landscape = file.path(data_path, "landscape"),
