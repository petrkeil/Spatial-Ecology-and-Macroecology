...
![](https://github.com/ropensci/MODIStsp/raw/master/man/figures/logo.png){height="100"} `MODIStsp`: <https://github.com/ropensci/MODIStsp/> (**NOT WORKING ATM**)
...
:::{.fragment}
[Check [`download_amphibiansPE_data_from_GBIF.R`](https://github.com/petrkeil/Spatial-Ecology-and-Macroecology/blob/main/Practical_classes/Week3_predictors/download_amphibiansPE_data_from_GBIF.R) to see how the data were downloaded, and [`preparing_amphibiansPE_grid.R`](https://github.com/petrkeil/Spatial-Ecology-and-Macroecology/blob/main/Practical_classes/Week3_predictors/preparing_amphibiansPE_grid.R) to see how I created the grid]{.font-aside}   
:::
...
r <- rast(res = 0.5, # target cell size (0.5 degree)
...
footp <- resample(footp, r, method = "bilinear")
...
The variables `tavg` and `prec` have 12 layers (one per month). We will keep the average for the year.
  ...
  tm_raster(col_alpha = 0.7,
    col.scale = 
      tm_scale_categorical(
        values = "brewer.set3"), 
    col.legend = tm_legend('landcover')
    )
```
  ...
  tm_raster(col_alpha = 0.7,
    col.scale = 
      tm_scale_continuous(
        values = "viridis", 
        n = 10), 
    col.legend = tm_legend('footprint')
    )
```
...
predictors <- tibble(
  "tavg" = values(tavg) %>% as.vector(),
  "prec" = values(prec) %>% as.vector(),
...
See the solutions at [practical_week3_STUDENTS_FILE_solved.qmd](https://github.com/petrkeil/Spatial-Ecology-and-Macroecology/blob/main/Practical_classes/Week3_predictors/practical_week3_STUDENTS_FILE_solved.qmd).
