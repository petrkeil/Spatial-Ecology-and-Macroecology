...
An example of large-scale biodiversity database, learning the basics and visualizing things. Gridding biodiversity data (geographic ranges, biodiversity), exploring patterns at different resolutions, making pretty maps.
...
#| echo: true
#| eval: false
testudines_grid <- st_join(world_grid, testudines_ea) %>%
  group_by(gridID) %>%
  summarise(N=n(),
            SR=n_distinct(BINOMIAL))
```
...
#| eval: false
ggplot() + 
  geom_sf(data=testudines_grid, aes(fill=SR)) +
  scale_fill_fermenter(palette ='YlOrBr', n.breaks=9, direction = 1) +
  geom_sf(data=world_ea, fill=NA) +
  coord_sf(crs=4326) +
  theme_bw()
```
