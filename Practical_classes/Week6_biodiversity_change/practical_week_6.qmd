...
for(i in 1:nrow(unique(species[,c("CountryNum", "StateNum", "Route")]))){
  ...
  for(year in unique(road_data$Year)[-1]){
    
    ## get the species at time t
    sptimet <- road_data[road_data$Year == year,] %>% pull(AOU)
    
    ## get the species at time t-1
    sptimet_1 <- road_data[road_data$Year == (year-1),] %>% pull(AOU)
    
    ## colonization
    col <- length(sptimet[!(sptimet %in% sptimet_1)])
    
    ## extinction
    ext <- length(sptimet_1[!(sptimet_1 %in% sptimet)])
    
    ## persistence
    pers <- length(sptimet_1[sptimet_1 %in% sptimet])
...
col_ext <- readRDS("data/col_ext.rds") %>% filter(ext != 0)
head(col_ext)
```
...
-   **Task 1:** Assess $\Delta S$ at the scale of North America using a simple linear regression. For this, you will have to assess the number of unique species for each year in all the BBS routes (using the `group_by()` function on the `species` data). How has bird richness changed between 2000 and 2021 in the US? Plot the time series and the fitted linear regression.

```{r, echo=F}
## Compute species richness for North America
sr_NA <- species %>% 
  group_by(Year) %>% 
  summarise(speciesRichness = length(unique(AOU)))

## Fit simple linear regression to assess delta S
lm_NA <- lm(speciesRichness ~ Year, data = sr_NA)

## Plot the time series
sr_NA %>% 
  ggplot()+
  geom_point(aes(x = Year, y = speciesRichness))+
  geom_line(aes(x = Year, y = lm_NA$fitted.values), color = "blue")+
  theme_bw()
```

-   **Task 2:** Assess $\Delta S$ at the scale of North America using a polynomial linear regressions of different order. Which polynomial fits the best the data? Plot the times series together with the different polynomial regressions.

```{r, echo=F}
## Fit polynomials
poly_2 <- lm(speciesRichness ~ poly(Year, 2), data = sr_NA)
poly_3 <- lm(speciesRichness ~ poly(Year, 3), data = sr_NA)
poly_4 <- lm(speciesRichness ~ poly(Year, 4), data = sr_NA)

## ASsess the best model
paste0("R² poly1 = ", summary(lm_NA)$adj.r.squared)
paste0("R² poly2 = ", summary(poly_2)$adj.r.squared)
paste0("R² poly3 = ", summary(poly_3)$adj.r.squared)
paste0("R² poly4 = ", summary(poly_4)$adj.r.squared)
...
## Plot them
ggplot()+
  geom_point(aes(x = Year, y = speciesRichness), data = sr_NA)+
  geom_line(aes(x = sr_NA$Year, y = lm_NA$fitted.values , colour = "poly_1"))+
  geom_line(aes(x = sr_NA$Year, y = poly_2$fitted.values, colour = "poly_2") )+
  geom_line(aes(x = sr_NA$Year, y = poly_3$fitted.values, colour = "poly_3") )+
  geom_line(aes(x = sr_NA$Year, y = poly_4$fitted.values, colour = "poly_4") )+
  theme_bw()

```

-   **Task 3:** Compute the average colonization and average extinction over the time-series for each BBS route (using `group_by()` and `summarise()`). Map those 2 values.
...
routes_shp <-
  ...
  left_join(col_ext %>% 
              group_by(CountryNum, StateNum, Route) %>% 
              summarise(average_col = mean(col),
                        average_ext = mean(ext),
                        StateNum = mean(as.numeric(StateNum)),
                        Route = mean(as.numeric(Route))),
            by = c("CountryNum", "StateNum", "Route"))

ggplot()+
  geom_sf(data = countries_shp)+
  geom_sf(aes(color = average_col), data = na.omit(routes_shp))+
  scale_color_viridis_c()+
  ggtitle("Average colonization")+
  theme_bw()+
  theme(plot.title = element_text(hjust = .5))

ggplot()+
  geom_sf(data = countries_shp)+
  geom_sf(aes(color = average_ext), data = na.omit(routes_shp))+
  scale_color_viridis_c()+
  ggtitle("Average extinction")+
  theme_bw()+
  theme(plot.title = element_text(hjust = .5))
```

-   **Task 4:** Now, for every route, assess the temporal change of colonization and extinction using linear regressions. Display those values on 2 histograms. 
...
col_trend <-
  ...
  group_by(CountryNum, StateNum, Route) %>%  ## those 3 columns represent the route id
  do({
      mod <- lm(col ~ year, data = .)                               #### fit the linear model 
      data.frame(slope = coef(mod)[2],                             #### extract the slope
                 intercept = coef(mod)[1])                         #### extract the intercept
      })

hist(col_trend$slope)
...
ext_trend <-
  ...
  group_by(CountryNum, StateNum, Route) %>%  ## those 3 columns represent the route id
  do({
      mod <- lm(ext ~ year, data = .)                               #### fit the linear model 
      data.frame(slope = coef(mod)[2],                             #### extract the slope
                 intercept = coef(mod)[1])                         #### extract the intercept
      })

hist(ext_trend$slope)
```
