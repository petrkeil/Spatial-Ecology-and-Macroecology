...
Polynomial regression is a specific case of a linear regression. Even though the final function
is not linear, the coefficients of each order of $x$ are linear:
...
species_richness %>% 
  ...
  theme_bw()
```
As you can see, this time the relationship does not really seem linear. So let's fit
polynomial regressions (*i.e.* quadratic, cubic...) and see how the regressions look like. 
...
We have seen that we can compute the temporal trend of a species richness by fitting a linear regression on it.
However, the variation of species richness $\Delta SR$:

$$\Delta SR = SR_{t_2}-SR_{t_1}$$
Which is the difference in the number of species between 2 time steps. We can assess $SR_{t_1}$
and $SR_{t_2}$ by using colonization $C$, extinction $E$ and persistence $P$. We can rewrite $\Delta SR$ as:
...
```{r}
## Load the colonization/extinction data 
